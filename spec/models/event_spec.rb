...
RSpec.describe Event do
  describe "#time_string" do
    context "when the event occurs on a single day" do
      it "prints a well formatted time" do
        event_start = Time.new(2015, 11, 19, 12, 30)
        event_end = Time.new(2015, 11, 19, 14, 30)
    ...
    context "when the event spans multiple days" do
      event_start = Time.new(2015, 11, 19, 12, 30)
      event_end = Time.new(2015, 11, 20, 14, 30)
  ...
  describe "scopes" do
    let(:past_event) { create(:event, start: Time.new(2015, 11, 9, 12, 30), end: Time.new(2015, 11, 9, 2, 30)) }
    let(:ongoing_event) { create(:event, start: Time.new(2015, 11, 10, 12, 30), end: Time.new(2015, 11, 10, 13, 30)) }
    let(:upcoming_event) { create(:event, start: Time.new(2015, 11, 11, 12, 30), end: Time.new(2015, 11, 11, 13, 30)) }

    before do
      current_time = Time.new(2015, 11, 10, 13, 00)
