...
RSpec.describe Group, type: :model do
  ...
  describe 'test associations and validations' do
    ...
    it { expect(group).to have_many(:fields).dependent(:delete_all) }
    ...
    # it { expect(group).to validate_length_of(:company_video_url).is_at_most(191) }
    it { expect(group).to validate_length_of(:sponsor_image_content_type).is_at_most(191) }
    it { expect(group).to validate_length_of(:sponsor_image_file_name).is_at_most(191) }
    it { expect(group).to validate_length_of(:calendar_color).is_at_most(191) }
    it { expect(group).to validate_length_of(:banner_content_type).is_at_most(191) }
    it { expect(group).to validate_length_of(:banner_file_name).is_at_most(191) }
    ...
    it { expect(group).to validate_length_of(:logo_content_type).is_at_most(191) }
    it { expect(group).to validate_length_of(:logo_file_name).is_at_most(191) }
  ...
  describe '#Enumerize' do
    ...
    it { expect(group).to enumerize(:members_visibility).in(:global, :group, :managers_only).with_default(:managers_only) }
    it { expect(group).to enumerize(:event_attendance_visibility).in(:global, :group, :managers_only).with_default(:managers_only) }
    it { expect(group).to enumerize(:messages_visibility).in(:global, :group, :managers_only).with_default(:managers_only) }
