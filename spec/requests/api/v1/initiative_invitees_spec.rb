...
RSpec.describe "#{model.pluralize}", type: :request do
  ...
  describe '#show' do
    ...
    it 'JSON body response contains expected attributes', skip: 'no serializer' do
      expect(JSON.parse(response.body)['initiative_invitee']).to include('id' => item.id)
    end
  ...
  describe '#create' do
    ...
    it 'contains expected attributes', skip: 'no serializer' do
      id = JSON.parse(response.body)['initiative_invitee']['id']
  ...
  describe '#update' do
    ...
    let!(:new_params) { { id: item.id, user_id: new_user.id } }
    ...
    it 'contains expected attributes' do
      expect(model.constantize.find(item.id).user_id).to eq new_params[:user_id]
    end
