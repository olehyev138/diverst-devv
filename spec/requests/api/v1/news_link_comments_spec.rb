...
RSpec.describe "#{model.pluralize}", type: :request do
  ...
  let!(:item) { create(model.constantize.table_name.singularize.to_sym, author: user, news_link: news_link) }
  ...
  describe '#show' do
    ...
    it 'JSON body response contains expected attributes' do
      expect(JSON.parse(response.body)['news_link_comment']).to include('id' => item.id)
    end
  ...
  describe '#create' do
    ...
    it 'contains expected attributes' do
      id = JSON.parse(response.body)['news_link_comment']['id']
  ...
  describe '#update' do
    let!(:new_params) { { id: item.id, content: 'test' } }
    before do
