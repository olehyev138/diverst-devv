...
model = 'InitiativeParticipatingGroup'
RSpec.describe "#{model.pluralize}", type: :request do
  ...
  describe '#show' do
    ...
    it 'JSON body response contains expected attributes', skip: 'no serializer' do
      expect(JSON.parse(response.body)['initiative_participating_group']).to include('id' => item.id)
    end
  ...
  describe '#create' do
    ...
    it 'contains expected attributes', skip: 'no serializer' do
      id = JSON.parse(response.body)['initiative_participating_group']['id']
