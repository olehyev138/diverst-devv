...
RSpec.describe 'InitiativeComments', type: :request do
  ...
  let!(:item) { create(model.constantize.table_name.singularize.to_sym, user: user) }
  ...
  describe '#show' do
    ...
    it 'JSON body response contains expected attributes' do
      expect(JSON.parse(response.body)['initiative_comment']).to include('id' => item.id)
    end
  ...
  describe '#create' do
    ...
    it 'contains expected attributes' do
      id = JSON.parse(response.body)['initiative_comment']['id']
