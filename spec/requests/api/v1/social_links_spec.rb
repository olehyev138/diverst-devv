...
RSpec.describe "#{model.pluralize}", type: :request do
  ...
  let!(:item) { create(:social_link, group: group, author: user) }
  ...
  describe '#show' do
    ...
    it 'JSON body response contains expected attributes' do
      expect(JSON.parse(response.body)['social_link']).to include('id' => item.id)
    end
  ...
  describe '#create' do
    ...
    it 'contains expected attributes' do
      id = JSON.parse(response.body)['social_link']['id']
  ...
  describe '#update' do
    ...
    it 'contains expected attributes' do
      expect(model.constantize.find(item.id).url).to eq new_params[:url]
    end
