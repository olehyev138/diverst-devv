...
RSpec.describe "#{model.pluralize}", type: :request do
  ...
  let!(:item) { create(:api_key) }
  let(:user) { create(:user, password: 'password', enterprise: enterprise) }
  let(:group) { create(:group, enterprise: enterprise) }
  let(:route) { model.constantize.table_name }
  let(:jwt) { UserTokenService.create_jwt(user) }
  let(:headers) { { 'HTTP_DIVERST_APIKEY' => item.key, 'Diverst-UserToken' => jwt } }
  ...
  describe '#create' do
    it 'creates an item' do
      post "/api/v1/#{route}", params: { "#{route.singularize}" => { key: SecureRandom.hex(8) } }, headers: headers
