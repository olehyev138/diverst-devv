...
RSpec.describe RewardsController, type: :controller do
  ...
  let(:reward_action){ create(:reward_action, enterprise_id: enterprise.id, points: 10) }
  ...
  describe 'PATCH #update' do
    context 'with logged in user' do
      ...
      context 'with correct params' do
        before :each do
          request.env["HTTP_REFERER"] = "update_reward_actions"
          patch :update_reward_actions, enterprise: {
        ...
        it 'redirects to back page' do
          expect(response).to redirect_to "update_reward_actions"
        end
      end
