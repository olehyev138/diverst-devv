    ...
    describe "PATCH#update" do
        describe "with logged in user" do
            ...
            context "with valid parameters" do
                attributes = FactoryGirl.attributes_for(:enterprise, home_message: "home message")
                before { patch :update, id: enterprise.id, enterprise: attributes }

                it "updates the enterprise" do
                    enterprise.reload
                    expect(assigns[:enterprise].home_message).to eq "home message"
                end
            ...
            context "with invalid parameters", skip: "render params['source'] causes ActionView::MissingTemplate" do
                before { patch :update, id: enterprise.id, enterprise: { name: "" } }

                it "does not update the enterprise" do
                    enterprise.reload
                    expect(enterprise.name).to eq "test"
                end
        ...
        describe "without a logged in user" do
           before { patch :update, id: enterprise.id, enterprise: attributes_for(:enterprise, name: "updated") }
            it_behaves_like "redirect user to users/sign_in path"
        end
    end
