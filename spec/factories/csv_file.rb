...
FactoryBot.define do
  factory :csv_file do
    ...
    factory :csv_file_with_download do
      user

      transient do
        download_file_file { Pathname.new("#{Rails.root}/spec/fixtures/files/diverst_csv_download.csv") }
        after(:build) do |csv_file_with_download, evaluator|
          if csv_file_with_download.import_file.attached?
            csv_file_with_download.import_file.detach
          end
          csv_file_with_download.download_file.attach(
