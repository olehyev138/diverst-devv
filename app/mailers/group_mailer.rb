class GroupMailer < ApplicationMailer
  def group_message(message)
    ...
    mail(to: message.users.first.email, bcc: message.users.pluck(:email), subject: @subject)
  end
  ...
  def invitation(group, _invitation_segments)
    ...
    @content = template.render('group' => {
                                 'image' => view_context.image_tag(group.logo.url(:thumb)),
                                 'name' => group.name,
                                 'description' => group.description
                               },
                               'accept_link' => join_user_group_url(group)).html_safe
