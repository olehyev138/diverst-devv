module AdminViewHelper
  def active_manage_erg_link?
    ...
    ['resources', 'segments', 'metrics_dashboards'].include? controller_name
  end
  ...
  def active_global_settings_link?
    ...
    return true if controller_name == 'enterprises' &&
      ['edit_auth', 'edit_fields', 'edit_branding'].include?(action_name)

    ['users', 'integrations', 'policy_groups', 'emails'].include? controller_name
  end
end
