module ApplicationHelper
  ...
  def group_initiative_expenses_link(text, group, initiative)
    return link_to text, group_initiative_expenses_path(group, initiative) if group.available_budget != 0

    return link_to text, group_initiative_expenses_path(group, initiative),
                   data: { confirm: 'you can not make any expenses because no budget has been approved' } if group.available_budget <= 0
  end
