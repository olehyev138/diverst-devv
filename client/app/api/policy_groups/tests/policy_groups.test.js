...
describe('PolicyGroup API Calls', () => {
  ...
  it('calls GET function', async () => {
    ...
    const payload = {
      page: 1, count: 10, order: 'desc', orderBy: 'policy_groups.created_at'
    };
    const page = await policyGroups.all(payload);

    expect(page).toEqual(response);
    expect(mockAxios.get).toHaveBeenCalledTimes(1);
    expect(mockAxios.get).toHaveBeenCalledWith('/api/v1/policy_groups?page=1&count=10&order=desc&orderBy=policy_groups.created_at');
  });
  ...
  it('calls GET function', async () => {
    ...
    expect(page).toEqual(response);
    expect(mockAxios.get).toHaveBeenCalledTimes(1);
    expect(mockAxios.get).toHaveBeenCalledWith('/api/v1/policy_groups/1');
  });
  ...
  it('calls POST function', async () => {
    ...
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/policy_groups', {});
  });
  ...
  it('calls PUT function', async () => {
    ...
    expect(mockAxios.put).toHaveBeenCalledWith('/api/v1/policy_groups/1', {});
  });
  ...
  it('calls DELETE function', async () => {
    ...
    expect(mockAxios.delete).toHaveBeenCalledWith('/api/v1/policy_groups/1');
  });
});
