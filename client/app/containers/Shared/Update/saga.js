...
export function* getUpdate(action) {
  try {
    ...
    yield put(showSnackbar({ message: messages.snackbars.errors.get_update, options: { variant: 'warning' } }));
  }
}
...
export function* getUpdates(action, updatableKey) {
  try {
    ...
    yield put(showSnackbar({ message: messages.snackbars.errors.updates, options: { variant: 'warning' } }));
  }
}
