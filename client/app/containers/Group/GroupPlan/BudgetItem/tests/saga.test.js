...
describe('Tests for budgetItems saga', () => {
  describe('Get budgetItems Saga', () => {
    ...
    it('Should return error from the API', async () => {
      ...
      expect(api.budgetItems.all).toHaveBeenCalledWith(initialAction.payload);
      expect(dispatched).toEqual(results);
      expect(Notifiers.showSnackbar).toHaveBeenCalledWith({ message: messages.snackbars.errors.budget_items, options: { variant: 'warning' } });
    });
  });
  ...
  describe('Get budgetItem Saga', () => {
    ...
    it('Should return error from the API', async () => {
      ...
      expect(api.budgetItems.get).toHaveBeenCalledWith(initialAction.payload.id);
      expect(dispatched).toEqual(results);
      expect(Notifiers.showSnackbar).toHaveBeenCalledWith({ message: messages.snackbars.errors.budget_item, options: { variant: 'warning' } });
    });
  });
  describe('close budget', () => {
    ...
    it('Should return error from the API', async () => {
      ...
      expect(api.budgetItems.closeBudget).toHaveBeenCalledWith(initialAction.payload.id);
      expect(dispatched).toEqual(results);
      expect(Notifiers.showSnackbar).toHaveBeenCalledWith({ message: messages.snackbars.errors.close, options: { variant: 'warning' } });
    });
  });
});
